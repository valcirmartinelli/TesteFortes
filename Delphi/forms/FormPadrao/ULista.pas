...
begin
  ...
  begin
    Application.MessageBox('Não há registros para alterar.', 'Atenção', MB_OK);
    exit;
  end;
end;
...
begin
  ...
  if LDataSet.RecordCount > 0 then
  begin
    if Application.MessageBox('Deseja excluir o registro?', 'Atenção', MB_YESNO + MB_ICONQUESTION) = IDYES then
    begin
      if self.ExcluirRegistro then
      begin
        Self.Pesquisar;
        Application.MessageBox('Registro Excluido com sucesso.', 'Atenção', MB_OK);
      end;
    end;
  end
  else
    Application.MessageBox('Não há registros para excluir.', 'Atenção', MB_OK);
end;
